...
function App() {
  ...
  return (
    <div
      className="min-h-screen w-screen flex flex-col items-center justify-center overflow-hidden bg-[#2d1a0d] bg-center bg-cover"
      style={{ backgroundImage: "url('/wood-material-background-wallpaper-texture-concept.jpg')" }}
      tabIndex={0}
      onKeyDown={handleKeyDown}
    >
      {!showLetter && (
        <h1 className="text-pink-500 font-pacifico mb-6 tracking-wide text-4xl drop-shadow-lg">
          Love Letter Generator
        </h1>
      )}
      {!showLetter ? (
        <form onSubmit={handleSubmit} className="bg-white/90 p-8 rounded-3xl shadow-2xl max-w-md w-[94vw] border-2 border-yellow-300 flex flex-col gap-4">
          <label className="text-[#b05d5e] font-bold text-left">
            To
            <input type="text" value={to} onChange={e => setTo(e.target.value)} required className="w-full p-2 rounded-lg border border-yellow-300 font-sans bg-yellow-50 text-[#b05d5e] mt-1" />
          </label>
          <label className="text-[#b05d5e] font-bold text-left">
            Message
            <textarea value={message} onChange={e => setMessage(e.target.value)} rows={6} required className="w-full p-2 rounded-lg border border-yellow-300 font-sans bg-yellow-50 text-[#b05d5e] mt-1" />
          </label>
          <label className="text-[#b05d5e] font-bold text-left">
            From
            <input type="text" value={from} onChange={e => setFrom(e.target.value)} required className="w-full p-2 rounded-lg border border-yellow-300 font-sans bg-yellow-50 text-[#b05d5e] mt-1" />
          </label>
          <button type="submit" className="bg-[#b05d5e] text-white rounded-lg py-3 text-lg font-pacifico shadow hover:bg-[#7c3a3a] transition">Generate Letter</button>
        </form>
      ) : (
        <div
          className="fixed inset-0 flex items-center justify-center z-10 bg-transparent"
        ...
        >
          ...
          >
            ...
            >
              ...
              <button onClick={handleEdit} className="absolute top-4 right-4 bg-white text-pink-700 border border-yellow-300 rounded px-3 py-1 font-pacifico cursor-pointer text-base">Edit</button>
              {pages.length > 1 && (
